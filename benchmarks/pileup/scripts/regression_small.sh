...
inputs_path="$GENARCH_BENCH_INPUTS_ROOT/pileup/small"
...
command_opts="\"$inputs_path/saureus.bam\" tig00000061:1-1499707 \$OMP_NUM_THREADS > out.txt"
...
after_run() (
    ...
    diff --brief "out.txt" "$inputs_path/out-reference.txt"
    if [[ $? -ne 0 ]]; then
