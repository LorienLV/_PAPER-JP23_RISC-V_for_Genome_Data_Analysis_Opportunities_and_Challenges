...
command_opts="eventalign -b "$inputs_path"/tiny/10reads.bam \
-g "$inputs_path"/humangenome.fa -r "$inputs_path"/10reads.fastq -B 3.7M \
...
after_run() (
    ...
    ndiffs="$(diff --speed-large-files \
              ...
              <(awk '{print $3$10}' "$inputs_path/tiny-reference.tsv") \
              | grep "^>" | wc -l)"
