...
QEMUVM)
    ...
    commands=(
        "$binaries_path/build_EPI_CPP/dbg"
    )
...
chr22:16000000-16005000 \"$inputs_path/large/Homo_sapiens_assembly38.fasta\" \
...
after_run() (
    ...
    if [[ $check_output -eq 1 ]]; then
        sort -n -k1,2 "$job_name.out" | diff --brief - "$inputs_path/tiny/reference.out" &>/dev/null
        if [[ $? -ne 0 ]]; then
